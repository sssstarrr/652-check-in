...
- 打卡：拉取待签到/已完成/缺勤任务，获取任务详情，判断已签到/无任务/异常，提交位置签到 JSON，并显示服务端记录的签到成功时间。
...
3. “失败重试间隔”默认为 5 分钟。没有任务、网络异常或提交失败时会继续重试；每个账号一旦签到成功或确认已签到，当天便跳过后续尝试。
4. 程序需要保持运行；关闭主窗口会隐藏到系统托盘，托盘菜单中的“退出”才会真正结束程序。电脑关机、休眠或程序退出时无法自动打卡。
5. 如果程序在设定时间之后启动，会在启动后补执行；旧版本遗留的“今日已执行”状态不会被当作成功记录。
6. 主界面右上角会显示自动打卡状态；签到成功时间会写入账号信息、日志和已完成任务的“签到时间”列。
