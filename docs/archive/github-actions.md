...
- `.github/workflows/daily-checkin.yml`：默认从北京时间 `16:01` 开始每 5 分钟唤醒一次，但只有 `19:31-23:55` 窗口内才真正打卡，也支持手动运行。
...
```yaml
schedule:
  # GitHub cron 使用 UTC，提前唤醒，再由 workflow 判断北京时间窗口。
  # 08:01-15:56 UTC = 16:01-23:56 北京时间。
  - cron: "1,6,11,16,21,26,31,36,41,46,51,56 8-15 * * *"
```

表示每天北京时间 `16:01` 到 `23:56` 每 5 分钟唤醒一次。workflow 会先判断当前北京时间，只有 `19:31-23:55` 内才继续打卡；更早的唤醒会直接跳过。这样可以抵消 GitHub schedule 的延迟。
...
GitHub Actions 定时任务不是秒级调度，可能延迟几分钟甚至更久。workflow 必须在默认分支上，仓库 Actions 也必须启用。默认配置会在北京时间 `16:01-23:56` 每 5 分钟唤醒一次，真正打卡窗口是 `19:31-23:55`，降低单个 cron 点没有及时触发的影响。
