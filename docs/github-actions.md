...
- `.github/workflows/daily-checkin.yml`：默认从北京时间 `19:31` 到 `20:31` 多次尝试，也支持手动运行。
...
```yaml
schedule:
  # GitHub cron 使用 UTC。这组重试覆盖北京时间 19:31-20:31。
  - cron: "31,41,51 11 * * *"
  - cron: "1,11,21,31 12 * * *"
```

表示每天北京时间 `19:31`、`19:41`、`19:51`、`20:01`、`20:11`、`20:21`、`20:31` 尝试运行。要改时间，编辑 `.github/workflows/daily-checkin.yml` 里的 `cron`，注意要换算成 UTC。
...
后续重试如果发现已经签到，会按“今日已签到”成功结束；这是正常的防漏机制。
...
GitHub Actions 定时任务不是秒级调度，可能延迟几分钟甚至更久。workflow 必须在默认分支上，仓库 Actions 也必须启用。默认配置会在北京时间 `19:31-20:31` 多次尝试，降低单次触发延迟的影响。
