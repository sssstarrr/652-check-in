...
- `.github/workflows/daily-checkin.yml`：每天 `19:31` 按北京时间运行一次，也支持手动运行。
...
```yaml
schedule:
  # GitHub cron 使用 UTC。11:31 UTC = 北京时间 19:31。
  - cron: "31 11 * * *"
```

表示每天北京时间 `19:31` 运行。要改时间，编辑 `.github/workflows/daily-checkin.yml` 里的 `cron`，注意要换算成 UTC。
...
GitHub Actions 定时任务不是秒级调度，可能延迟几分钟。workflow 必须在默认分支上，仓库 Actions 也必须启用。GitHub cron 使用 UTC，默认 `31 11 * * *` 对应北京时间 `19:31`。
